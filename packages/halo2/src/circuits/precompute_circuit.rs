use crate::chips::precompute_chip::{EfficientECDSAPrecomputeChip, EfficientECDSAPrecomputeConfig};
...
#[derive(Default)]
pub struct EfficientECDSAPrecomputeCircuit<E: CurveAffine, N: FieldExt> {
...
impl<E: CurveAffine, N: FieldExt> Circuit<N> for EfficientECDSAPrecomputeCircuit<E, N> {
    type Config = EfficientECDSAPrecomputeConfig;
    ...
    fn configure(meta: &mut ConstraintSystem<N>) -> Self::Config {
        EfficientECDSAPrecomputeChip::<E, N, NUMBER_OF_LIMBS, BIT_LEN_LIMB>::configure(meta)
    }

    fn synthesize(&self, config: Self::Config, layouter: impl Layouter<N>) -> Result<(), Error> {
        let chip = EfficientECDSAPrecomputeChip::<E, N, NUMBER_OF_LIMBS, BIT_LEN_LIMB>::new(config);
...
mod tests {
    ...
    use crate::ecdsa_helper::{
        bn_256_pkeygen, bn_256_prove, bn_256_vkeygen, generate_precompute_input,
    };
    ...
    fn test_mock() {
        ...
        let prover =
            MockProver::run::<EfficientECDSAPrecomputeCircuit<E, N>>(18, &circuit, instance)
                .unwrap();
    ...
    #[cfg(test)]
    #[test]
    fn test_full_prove() -> Result<(), Error> {
